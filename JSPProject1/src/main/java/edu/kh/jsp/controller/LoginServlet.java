...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// * 응답화면을 만드는 Servlet 의 일을 
		// 좀 더 효율적으로 처리할 수 있는 JSP 에게 넘겨줄 예정 
		
		// RequestDispatcher : 
		// Servlet => JSP로
		// HttpServletRequest 객체 , HttpServletResponse 객체를
		// 발송(위임)하는 역할의 객체
		// => 정확히는 요청에대한 응답화면을 만들어
		...
		// - value 는 모든 객체 (타입 상관 없음)
		// Attribute : 속성 == 데이터(값)
		
		req.setAttribute("r", result);
							// Object 업캐스팅 되어 있는 상태
		
		
		// -forward는 페이지 이동이 아닌
		// Servlet 의 역할 중 응답화면출력 역할을 분업산 JSP에게
		// req , resp 를 전달만 하는 것
		// => JSP 응답을 대신 할 뿐이다.
