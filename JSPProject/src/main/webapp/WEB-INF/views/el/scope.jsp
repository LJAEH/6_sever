...
<head>
    ...
    <title>2. ServletJSP 내장객체와 범위 (scope) </title>
</head>
<body>
	...
	<pre> 
		...
		1. page : 현재 페이지
				 => 현재 Servlet 또는 JSP 에서만 사용 가능 (1페이지)
		2. request : 요청받은 페이지(Servlet/JSP) 와
					요청을 위임받은 페이지(Servlet/JSP)에서 사용 가능
					(최소 2페이지 이상)
		3. session : 현재 사이트에 접속한 브라우저당 1개씩 생성
					브라우저가 종료 되거나 session 이 만료될 때까지 유지
					(세션에 로그인 정보를 기록해둠 => 브라우저가 종료되거나 
					로그아웃 되기 전까지 계속 로그인 상태 유지)
		4. application : 하나의 web application 당 1개만 생성되는 객체.
		...
		***************************************************
		...
		=> setAttribute("key",value)로 내장 객체가 값을 세팅할 때
			...
			\${key}로 작성하는 경우
			범ㅁ위가 작은 내장 객체가 높은 우선 순위를 갖게 된다.
