...
<!-- 
	prefix : 접두사
	uri(Uniform Resouce Identifier) : 자원을 구분하는 식별자
 -->
...
<body>
	...
	<pre>
		...
		(if , for , 배열/컬렉션 길이, 문자열 치환, 데이터 형식 변경 , 데이터 파싱 
		scope 변수 선언 등)
	</pre>
	...
	<ol>
		<li>http://tomcat.apache.org/download-taglibs.cgi </li>
    ...
    <pre>
    	...
    	- c:set 속성
    	1) var : 변수명(setAttribute 의 key 값)
    	2) value : 대입될 값
	...
	<%-- session 범위에 세팅된 test 의 값이 "session" 인 경우 --%>
	<%--
		- 문자열 비교시 (==) 비교연산자 사용 가능
		- 문자열임을 표기하는 리터럴 == ''(홀따옴표) / "" (쌍따옴표)
	 ...
	 <pre>
	 	c:choose : when, otherwise 를 감싸는 태그
	 			(현재 태그 내부에 if-else 를 작성하겠다.)
